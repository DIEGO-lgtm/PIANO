...
const int HIT_ZONE_TOP = 700;
const int HIT_ZONE_BOTTOM = 760;
...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(400, 600), "Piano Arrolladora");
    ...
    while (running) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                ...
                if (pressed != '\0') {
                    for (auto& tile : tiles) {
                        if (tile.active &&
                            tile.y + TAMBOR_HEIGHT >= HIT_ZONE_TOP &&
                            tile.y <= HIT_ZONE_BOTTOM &&
                            tile.key == pressed) {
        ...
        sf::RectangleShape zonaGolpe(sf::Vector2f(400, HIT_ZONE_BOTTOM - HIT_ZONE_TOP));
        zonaGolpe.setPosition(0, HIT_ZONE_TOP);
        zonaGolpe.setFillColor(sf::Color(255, 255, 255, 40));
        window.draw(zonaGolpe);
        ...
        letrasGuia.setPosition(200 - letrasGuia.getLocalBounds().width / 2, 570);
